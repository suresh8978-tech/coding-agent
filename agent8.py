...
def create_agent(model_name: str | None = None):
    ...
    llm_kwargs = {
        "model": llm_name,
        "api_key": api_key,
        "max_tokens": 4096,  # slightly reduced to leave more room for input
    }
    if api_url:
...
def agent_node(state: AgentState) -> dict:
    ...
    if not messages or not isinstance(messages[0], SystemMessage):
        ...
        if agent_md_content:
            agent_md_trimmed = agent_md_content[:MAX_AGENT_MD_CHARS]
            if len(agent_md_content) > MAX_AGENT_MD_CHARS:
                agent_md_trimmed += (
                    f"\n\n[AGENT.md truncated to {MAX_AGENT_MD_CHARS} characters "
                    "to fit model context.]"
                )
            system_content += build_agent_md_context(agent_md_trimmed)
    ...
    # If awaiting modification approval, add compact context
    if state.get("awaiting_modification_approval"):
        ...
        if pending:
            ...
            changes_text = format_changes_for_display(pending)
            if len(changes_text) > 8000:
                changes_text = (
                    changes_text[:8000]
                    + "\n\n[Pending change display truncated to keep within context.]"
                )
            context += changes_text
    ...
    if state.get("awaiting_push_approval"):
        ...
        context = (
            f"\n\n[SYSTEM: Changes have been applied. Awaiting push approval "
            f"for branch '{branch}'.]\n"
        )
        messages = messages + [SystemMessage(content=context)]

    # Truncate history to avoid exceeding Sonnet 4.5 context
    messages = _truncate_messages_for_llm(messages)
...
def tools_node(state: AgentState) -> dict:
    ...
    # Truncate large tool outputs to keep model input manageable
    trimmed_messages = []
    for msg in all_result_messages:
        if isinstance(msg, ToolMessage):
            msg = _truncate_tool_message_content(msg)
        trimmed_messages.append(msg)
    all_result_messages = trimmed_messages
...
def _truncate_messages_for_llm(messages: list) -> list:
    """Limit conversation history so Bedrock Sonnet 4.5 doesn't exceed context."""
    if not messages:
        return messages
    ...
    trimmed = system_msgs + non_system[-MAX_HISTORY_MESSAGES:]
    return trimmed


def _truncate_tool_message_content(msg: ToolMessage) -> ToolMessage:
    """Ensure tool outputs don't blow up the context."""
    content = msg.content
    if isinstance(content, str) and len(content) > MAX_TOOL_CHARS:
        trimmed = (
            content[:MAX_TOOL_CHARS]
            + f"\n\n[Tool output truncated to {MAX_TOOL_CHARS} characters "
...
def build_agent_md_context(agent_md_content: str | None) -> str:
    ...
    context += "\n--- AGENT.md ---\n"
    context += agent_md_content[:30000]
...
def build_context_message(mop_content: dict | None) -> str:
    ...
    full_text = mop_content.get("full_text", "")
    full_text_trimmed = full_text[:MAX_MOP_CHARS]
    if len(full_text) > MAX_MOP_CHARS:
        full_text_trimmed += (
            f"\n\n[MOP content truncated to {MAX_MOP_CHARS} characters "
            "to fit model context.]"
        )
...
def run_single_query(query: str, repo_path: str | None = None, mop_path: str | None = None) -> str:
    ...
    try:
        ...
        return (
            ...
            f"{interrupt_value}\n\n"
            "Note: Cannot approve pushes in non-interactive mode. "
