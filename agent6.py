...
class AgentState(TypedDict):
    ...
    # Read-only mode - disables code modification and git push
    read_only: bool
...
- Always show ALL proposed changes inside ```diff code blocks, including new files. For new files, prefix every line with + to show it as an addition. For modifications, use - for removed lines and + for added lines. NEVER use ```yaml or ```python blocks for proposed changes - ALWAYS use ```diff
...
def agent_node(state: AgentState) -> dict:
    ...
    # Tell the LLM not to use write tools in read-only mode
    if state.get("read_only"):
        messages = messages + [HumanMessage(content=(
            "[SYSTEM: You are in READ-ONLY mode. You can read files, analyze code, "
            "and answer questions, but you CANNOT modify files, create branches, "
            "commit, or push. Do not attempt to use write_file, git_push, or any "
            "modification tools.]"
        ))]
    ...
    # If messages are still too large, aggressively turncate tool results
    total_chars = sum(len(str(m.content)) for m in messages if hasattr(m, 'content'))
    if total_chars > 600000: # ~150K tokens
        logger.warning(f"Messages too large ({total_chars} chars), aggresively truncating")
        trimmed = []
        for m in messages:
            if isinstance(m, ToolMessage) and isinstance(m.content, str) and len(m.content) > 2000:
...
def setup_node(state: AgentState, config: RunnableConfig) -> dict:
    ...
    logger.info(f"Full configurable received: {configurable}")
    user_email = configurable.get("user", "")
    
    if user_email:
        os.environ["GIT_AUTHOR_EMAIL"] = user_email
        os.environ["GIT_COMMITTER_EMAIL"] = user_email
    
    logger.info(f"GIT_AUTHOR_EMAIL set to: {os.environ.get('GIT_AUTHOR_EMAIL', 'not set')}")

    updates = {}

    # Set read-only mode from config
    readonly_mode = configurable.get("readonly_mode", False)
    logger.info(f"readonly_mode: {readonly_mode}")
    if readonly_mode and readonly_mode == True:
        updates["read_only"] = True

    logger.info(f"readonly_mode set to: {readonly_mode or 'not set'}")    
...
def should_continue(state: AgentState) -> Literal["tools", "approval_check", "push_approval", "end"]:
    ...
    # Check if we need push approval
    if state.get("awaiting_push_approval"):
...
def tools_node(state: AgentState) -> dict:
    ...
    # Tools blocked in read-only mode
    ...
    for tc in last_message.tool_calls:
        if state.get("read_only") and tc["name"] in READ_ONLY_BLOCKED_TOOLS:
            logger.info(f"Blocking {tc['name']} in read-only mode")
            blocked_messages.append(
                ToolMessage(
                    tool_call_id=tc["id"],
                    content=f"[BLOCKED] {tc['name']} is not available in read-only mode. You can only read and analyze code.",
                    name=tc["name"]
                )
            )

        elif tc["name"] == "git_push" and not state.get("push_approved"):
            logger.info(f"Blocking unaaproved git_push call: {tc['id']}")
            blocked_push_call = tc
            blocked_messages.append(
                ToolMessage(
                    tool_call_id=tc["id"],
                    content="[BLOCKED] git_push requires approval. The system will now ask the user to confirm.",
                    name=tc["name"]
                )
            )    
        else:
...
def summarize_node(state: AgentState) -> dict:
    ...
    # Let's say max allowed tokens for history before we summarize is 80,000
    TOKEN_LIMIT = 50000
    ...
    if total_tokens > TOKEN_LIMIT and len(messages) > 6:
        # Leave at least 10 messages untouched at the end
        keep_at_least = 10
...
def create_graph():
    ...
    # --- tools → push_approval if push blocked, else summarize ---
    ...
    workflow.add_conditional_edges(
        "tools",
        tools_router,
        {"push_approval": "push_approval", "summarize": "summarize", "error_handler": "error_handler"},
    )
    ...
    # --- push_approval → execute_push or back to agent ---
    workflow.add_conditional_edges(
        "push_approval",
    ...
    try:
        while True:
            ...
            try:
                ...
                # Check if graph paused after invoke
                state_snapshot = graph.get_state(config)
                if state_snapshot.next:
                    interrupt_value = "Push approval required"
